...
public class searchFilm extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<Film> filmsLike = dao.getFilmByTitle(filmName);
		...
		if (format.equals("xml")) {
			...
			for (int i = 0; i < filmsLike.size(); i++) {
				try {
				...
				jaxbMarshaller.marshal(filmsLike.get(i), sw);

				// Verify XML Content
				String xmlContent = sw.toString();
				System.out.println(xmlContent);
				request.setAttribute("films", xmlContent);
			...
			for(int i = 0; i < filmsLike.size(); i++) {
				System.out.println(filmsLike.get(i).toString());
			}
			request.setAttribute("films", filmsLike);
			...
			Gson gson = new Gson();
			request.setAttribute("films", gson.toJson(filmsLike));

			for (int i = 0; i < filmsLike.size(); i++) {
				System.out.println(gson.toJson(filmsLike.get(i)));
			}
		}
